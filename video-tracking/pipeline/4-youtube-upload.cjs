 ...
 *   2. Create project "Discover Arizona"
...
function generateMetadata(filename) {
  ...
  if (type === 'dest') {
    return {
      title: `${title} Arizona Travel Guide ${year} | Real Prices & Local Tips`,
      ...
      playlist: `${dest?.region || 'arizona'}-destinations`,
    };
  }

  if (type === 'pillar') {
    return {
      title: `${title} — Arizona ${year}`,
      description: `${title}\n\nFull guide: https://discoverarizona.info/${slug}/`,
      tags: [`Arizona ${slug}`, 'Arizona travel', `Arizona ${year}`],
      categoryId: '19',
  ...
  if (type === 'short') {
    return {
      title: `${title} Arizona ✈️ #shorts`,
      description: `${title} — Full guide: https://discoverarizona.info/destinations/${slug}/`,
      tags: [`${title} Arizona`, 'Arizona travel', 'shorts'],
      categoryId: '19',
      privacyStatus: 'private',
      playlist: 'shorts',
    };
...
function generateDestDescription(slug, title, year) {
  return `Complete travel guide to ${title}, Arizona — with real prices, how to get there, where to eat, where to stay, and what most tourists miss.

I've been exploring Arizona for years with deep local knowledge. No sponsored content.

📖 FULL WRITTEN GUIDE: https://discoverarizona.info/destinations/${slug}/
📱 TRIP COMPANION (works offline): https://discoverarizona.info/companion/

#Arizona #${title.replace(/\s+/g, '')} #ArizonaTravel #TravelGuide${year}`;
}
...
function generateDestTags(slug, title, dest) {
  const tags = [
    `${title} Arizona`,
    `${title} travel guide`,
    `Arizona ${new Date().getFullYear()}`,
    `${title} things to do`,
    `where to stay ${title}`,
    'Arizona travel',
    'Arizona food',
    'Arizona budget travel',
    'Arizona tips',
  ];
  if (dest?.region) tags.push(`${dest.region} Arizona`);
...
async function main() {
  console.log('╔══════════════════════════════════════════════════════════╗');
  console.log('║   YOUTUBE UPLOAD — DISCOVER ARIZONA                ║');
  console.log('╚══════════════════════════════════════════════════════════╝\n');
