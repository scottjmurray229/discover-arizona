...
function extractDestsFromText(text: string): string[] {
  ...
  const themeMap: Record<string, string[]> = {
    'canyon': ['grand-canyon', 'page', 'sedona'],
    'desert': ['tucson', 'saguaro-national-park', 'phoenix'],
    'hiking': ['sedona', 'grand-canyon', 'flagstaff'],
    'history': ['tombstone', 'bisbee', 'flagstaff'],
    'photography': ['page', 'monument-valley', 'sedona'],
    'luxury': ['scottsdale', 'sedona', 'phoenix'],
    'road trip': ['grand-canyon', 'sedona', 'monument-valley'],
    'national park': ['grand-canyon', 'petrified-forest', 'saguaro-national-park'],
    'old west': ['tombstone', 'bisbee', 'flagstaff'],
    'nature': ['sedona', 'grand-canyon', 'saguaro-national-park'],
  };
  ...
  return ['grand-canyon', 'sedona', 'phoenix'];
}
...
function buildSystemPrompt(): string {
  return `You are the Discover Arizona AI Trip Planner. You generate detailed day-by-day itineraries for Arizona travel.
...
- Arizona context: desert heat, canyon terrain, elevation changes, scenic drives, national parks
- Tag hotel/tour/transport items with affiliateType and affiliateSlotId for future monetization
- affiliateSlotId format: "day{N}-{type}-{destination}" e.g. "day1-hotel-grand-canyon"
...
- 15+ days: 2-3 items per day, brief descriptions (under 15 words each). Group similar days (e.g. "Days 5-6: Exploring Sedona trails"). Only include key activities, one meal, and accommodation.
...
{
  "title": "string — trip title like '7-Day Arizona Canyon & Desert Adventure'",
  "subtitle": "string — budget + month like 'Mid-Range · March 2026'",
  "totalBudget": { "usd": number },
  "days": [
    {
      "dayNumber": 1,
      "title": "string — day title like 'Arrive in Phoenix'",
      "destination": "string — slug like 'phoenix'",
      "items": [
        {
          ...
          "locationName": "string — specific place name for map pin, e.g. 'Horseshoe Bend', 'Cathedral Rock', 'South Rim Visitor Center'. Use real, specific place names."
        }
      ]
    }
  ]
...
function buildUserPrompt(body: RequestBody, destinations: string, duration: string, budgetLevel: string): string {
  ...
  let prompt = `Plan a ${duration}-day Arizona trip:\n${parts.join('\n')}`;
